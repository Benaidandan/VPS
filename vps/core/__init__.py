...
class VisualPositioningSystem:
    ...
    def _vpr_task(self, query_image, last_pose):
        ...
        end_time = time.time()
        print(f"VPR time: {end_time - start_time} seconds")
    ...
    def localize(self,
        ...
        with ThreadPoolExecutor(max_workers=2) as executor:
            ...
            # 等待VPR完成
            similar_pairs = vpr_future.result()
            ...
            if depth_future:
                ...
                # 如果深度预测完成，使用预测的深度
                if depth_result is not None and 'depth' in depth_result:
                    # 保存预测的深度到临时文件
        ...
        b = time.time()
        print(f"Parallel VPR + Depth time: {b - a} seconds")
